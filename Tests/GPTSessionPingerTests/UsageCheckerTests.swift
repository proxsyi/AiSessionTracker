...
final class UsageCheckerTests: XCTestCase {
    ...
    func testModelLimitDrivesAvailabilityAndReset() throws {
        let reset = Date().addingTimeInterval(3_600)
        let object: [String: Any] = [
            "model_limits": [[
                "model_slug": "gpt-5-4-t-mini",
                "remaining": 0,
                "reset_at": reset.timeIntervalSince1970
            ]]
        ]
        let usage = GPTUsage(
            tracks: UsageChecker.parseChatGPTTracks(object),
            blockedFeatures: [],
            planType: "plus",
            fetchedAt: Date()
        )

        let availability = SessionAvailabilityResolver.resolve(
            usage: usage,
            model: "gpt-5-4-t-mini",
            availableModels: ["gpt-5-4-t-mini"],
            now: Date()
        )
        guard case .waiting(let actualReset) = availability else {
            return XCTFail("Expected a waiting availability")
        }
        XCTAssertEqual(actualReset.timeIntervalSince1970, reset.timeIntervalSince1970, accuracy: 0.01)
    }

    func testModelCatalogMeansAvailableWhenNoLimitIsReported() {
        let usage = GPTUsage(tracks: [], blockedFeatures: [], planType: "pro", fetchedAt: Date())
    ...
    func testModelCatalogParserUsesOnlyTopLevelVisibleModelsInServerOrder() {
        let object: [String: Any] = [
            "models": [
                ["slug": "gpt-b", "enabled": true],
                ["slug": "gpt-hidden", "hidden": true],
                ["slug": "o3", "enabled": true],
                ["slug": "gpt-a", "is_visible": true],
                ["slug": "claude-internal", "enabled": true]
            ],
            "categories": [["default_model": "gpt-internal"]]
        ]

        XCTAssertEqual(UsageChecker.parseAvailableModels(object), ["gpt-b", "o3", "gpt-a"])
    }
}
